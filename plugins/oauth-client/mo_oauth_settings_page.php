...
<div id="mo_oauth_settings">
	...
	if (get_option ( 'verify_customer' ) == 'true') {
		...
		if($currenttab == 'customization')
			...
			mo_oauth_faq(); 
		} else if($currenttab == 'login' && !mo_oauth_is_customer_registered()){ 
...
function mo_oauth_sign_in_settings(){
	...
	<div class="mo_oauth_premium_option_text"><span style="color:red;">*</span>This is a standard feature. 
		<a href="admin.php?page=mo_oauth_settings&tab=licensing">Click Here</a> to see our full list of Standard Features.</div>
...
function mo_oauth_app_customization(){
	...
	<?php if ( ! mo_oauth_is_customer_registered() ) { ?>
		<div style="display:block;margin-top:10px;color:red;background-color:rgba(251, 232, 0, 0.15);padding:5px;border:solid 1px rgba(255, 0, 9, 0.36);">Please <a href="<?php echo add_query_arg( array( 'tab' => 'login' ), htmlentities( $_SERVER['REQUEST_URI'] ) ); ?>">Register or Login with miniOrange</a> to configure the miniOrange OAuth client Plugin.</div>
	<?php } ?>
	
	<form disabled id="form-common" name="form-common" method="post" action="admin.php?page=mo_oauth_settings&tab=customization">
 ...
 .custom_logo{
	 ...
	 color:#fff;
			...
			<tr>
				...
				<td><input disabled type="text" style="resize: vertical; width:200px; height:30px;  margin:5% auto;" placeholder ="Howdy ,##user##" value=""><b>##user## is replaced by Username</b></td>
			</tr>
			<tr>
				<td>&nbsp;</td>
				<td><input disabled value="Save settings"
					class="button button-primary button-large" /></td>
			</tr>
		</table>
	</form>
...
function update_app($appname){
	...
	?>
		...
		<table class="mo_settings_table">
			...
			<tr>
				<td><strong>Display App Name:</strong></td>
				<td><input class="mo_table_textbox" type="text" name="mo_oauth_display_app_name" value="<?php echo isset($currentapp['displayappname']) ? $currentapp['displayappname'] : '';?>"></td>
			</tr>
