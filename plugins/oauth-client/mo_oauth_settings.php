<?php
/**
* Plugin Name: OAuth Client
* Plugin URI: http://miniorange.com
* Description: OAuth Login plugin allows login with your Strava, FitBit, Slack, Discord, Custom OAuth server, Openid Connect provider(Updating)
* Version: 1.6.1
* Author: miniOrange
...
class mo_oauth {
	...
	function plugin_settings_script() {
		...
		wp_enqueue_script( 'mo_oauth_admin_settings_script', plugins_url('js/bootstrap.min.js', __FILE__ ));
	}
	...
	function miniorange_oauth_save_settings(){
		...
		if( current_user_can( 'manage_options' ) && isset( $_POST['option'] )) {
			...
			else if( mo_oauth_is_customer_registered() && $_POST['option'] == "mo_oauth_add_app" ) {
				$scope = '';
				$clientid = '';
				$clientsecret = '';
